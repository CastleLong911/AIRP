...
class Variable(
    NamedTuple(
        "_Variable", [("name", str), ("nodes", Sequence[ast.AST]), ("value", Any)]
    )
):
    """
    ...
    def _raw_split_into_pieces(
        ...
        for name, body in ast.iter_fields(stmt):
            if (
                ...
                and isinstance(
                    body[0],
                    (ast.stmt, ast.ExceptHandler, getattr(ast, "match_case", ())),
                )
            ):
...
def style_with_executing_node(style, modifier):
    ...
    class NewStyle(style):
        ...
        styles = {
            **style.styles,
            **{k.ExecutingNode: v + " " + modifier for k, v in style.styles.items()},
        }
